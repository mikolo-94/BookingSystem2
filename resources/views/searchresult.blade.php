        ...
        <div class="row">
            ...
            <div class="bs-wizard" style="border-bottom:0; padding-bottom: 120px;">
                ...
                <div class="col-xs-3 bs-wizard-step disabled"><!-- complete -->
                    <div class="text-center bs-wizard-stepnum">Booking Information</div>
                    <div class="progress"><div class="progress-bar"></div></div>
                    <a href="#" class="bs-wizard-dot"></a>
                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-10 date">
                                    <div class="input-group input-append date" id="datePicker">
                                        <input type="text" class="form-control" name="start_dt" value="{{$start_dt}}" />
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-10 date">
                                    <div class="input-group input-append date" id="datePicker2">
                                        <input type="text" class="form-control" name="end_dt" value="{{$end_dt}}" />
                                        <span class="input-group-addon add-on"><span class="glyphicon glyphicon-calendar"></span></span>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="col-md-10 col-md-offset-1">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        @if($roomsVacants->isEmpty())
                            <h3>Tyvärr hittades inga rum som passar dina sökningar den valda perioden {{$start_dt}} till {{$end_dt}}</h3>
                        @endif

                        @foreach($roomsVacants as $roomsVacant)
                            <div class="well">
                                <form action="{{ route('bookingdetails') }}" method="post">
                                    ...
                                    <h3>{{$roomsVacant->roomtype->roomtype}}</h3>
                                    <p>{{$roomsVacant->roomtype->description}}</p>
                                   <h3>{{$roomsVacant->roomtype->base_price * $days }} SEK</h3> <h6>for {{$days}} nights</h6>
                                    <input type="hidden" name="roomtype_id" value="{{$roomsVacant->roomtype->id}}">
                                    <input type="hidden" name="start_dt" value="{{$start_dt}}">
                                    <input type="hidden" name="end_dt" value="{{$end_dt}}">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    <td><button type="submit" class="btn btn-default">Boka</button></td>
                                </form>
                            </div>
                        @endforeach
                        <script>
